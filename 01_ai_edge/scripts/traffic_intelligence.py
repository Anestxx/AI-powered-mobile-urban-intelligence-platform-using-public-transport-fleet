...
class TrafficIntelligence:
    ...
    def calculate_index(self, counts):
        ...
        # Light vehicles contribute slightly less.
        lmv_score = counts["lmv_count"] * 9

        # Pedestrians contribute a smaller amount.
    ...
    def analyze(self, detections):
        ...
        observation = {
            "timestamp": now,
            "vehicle_count": counts["vehicle_count"],
            "hmv_count": counts["hmv_count"],
            "lmv_count": counts["lmv_count"],
            "pedestrian_count": counts["pedestrian_count"],
        ...
        self.history = [
            ...
            if now - item["timestamp"] <= self.persistence_window
        ]

        # Average recent traffic state.
        recent = self.history[-self.required_observations:]

        avg_index = (
            sum(item["traffic_index"] for item in recent)
            / len(recent)
            if recent
            else 0
        )
        ...
        avg_vehicle_count = (
            sum(item["vehicle_count"] for item in recent)
            / len(recent)
            if recent
            else 0
        )

        congestion_observations = sum(
            1
            for item in recent
            if (
                item["traffic_index"]
                >= self.congestion_index_threshold
                or item["vehicle_count"]
                >= self.congestion_vehicle_threshold
            )
        )

        congestion_confirmed = (
            len(recent) >= self.required_observations
            and congestion_observations >= self.required_observations
        )
        ...
        can_alert = (
            now - self.last_congestion_alert
        ...
        if congestion_confirmed and can_alert:
            ...
            congestion_alert = {
                ...
                "source": "edge_traffic_intelligence",
            }
    ...
    def get_status(self):
        ...
        return {
            ...
            "pedestrian_count": latest["pedestrian_count"],
            "congestion_confirmed": (
                latest["traffic_index"]
                >= self.congestion_index_threshold
            ),
        }